...
public class PlayerTestMovement : MonoBehaviour
{
    ...
    void WalkDefualt()
    {
        ...
        if (body.velocity.x <= speed + 5 && (triggerWallJumpLeft == true || triggerWallJumpRight == true))
        {
            otherSpeed = 20;
        }
        ...
        if (bTimeSlowTrigger == true)
        {
            if (IsGroundedLeft(doNotCollide))
            {
                ...
                if (Input.GetMouseButtonDown(1))
                {
                    ...
                    otherSpeed = ArrowJumping().x * bounceMultiplier;
                    otherForces = new Vector2(ArrowJumping().x * bounceMultiplier, 0);
                    body.AddForce(new Vector2(ArrowJumping().x * bounceMultiplier, ArrowJumping().y * bounceMultiplier), ForceMode2D.Impulse);
                    triggerWallJumpLeft = true;
                    triggerWallJumpRight = false;
                }
            }
